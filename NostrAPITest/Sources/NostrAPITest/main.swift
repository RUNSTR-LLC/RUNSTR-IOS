...
print("=== NostrSDK 0.3.0 API Discovery Tests ===\n")
...
// Test 2: Event creation patterns
print("2. Testing Event creation:")
do {
    if let keypair = Keypair() {
        // Create a text note event
        let event = try NostrEvent(kind: .textNote, 
                                   content: "Test event from NostrSDK test", 
                                   signedBy: keypair)
        print("✓ NostrEvent created and signed successfully")
...
do {
    ...
    // Test subscription
    let filter = Filter(kinds: [.textNote], limit: 10)
